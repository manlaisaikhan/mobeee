...
export const UpcomingMovieList = () => {
  ...
  const getData = async () => {
    try {
      ...
      // ⏳ 2 секунд хүлээгээд дараа нь өгөгдөл гаргадаг
      setTimeout(() => {
        setUpcomingMovieList(jsonData.results.slice(0, 10));
        setLoading(false);
      }, 2000);
    } catch (err) {
  ...
  return (
    <div className="w-full flex flex-col items-start px-10 animate-fadeIn">
      ...
      <div className="grid grid-cols-5 gap-[60px] mt-6 max-sm:grid max-sm:grid-cols-2">
